...
Load3ds::Load3ds()
{
	...
	curObjData.verData = NULL;
	curObjData.indexData = NULL;
...
void * Load3ds::loadFile(const std::string& file)
{
	int lBytesRead = 0;
...
int Load3ds::ProcessNextObjectChunk(Chunk * aPreviousChunk)
{
	...
	while (aPreviousChunk->mBytesRead < aPreviousChunk->mLength)
	{
		...
		case OBJTRIMESH:
			...
			ProcessNextObjectChunk(mCurrentChunk);
			ComputeNormals();
			if(curObjData.indexData != NULL&& curObjData.verData != NULL)
			{
				m_Objects.push_back(curObjData);
				curObjData.indexData = NULL;
				curObjData.verData = NULL;
			}
			break;
...
int Load3ds::FillIndexBuffer(Chunk * aPreviousChunk)
{
	...
	//add my code
	if(curObjData.indexData== NULL)
	{
		curObjData.indexData = new unsigned long[lNumFaces * 3];
		curObjData.IndexCount = lNumFaces * 3;
	}
	unsigned short* indexData = (unsigned short*)mBuffer;
	int nDataIndex = 0;
...
int Load3ds::FillTexCoordBuffer(Chunk * aPreviousChunk)
{
	...
	//add my code
	float* VerData = (float*)mBuffer;
	if(curObjData.verData == NULL)
		curObjData.verData = new XYZNUV[lNumTexCoords];
	int nDataIndex = 0;
	for(short nIndex = 0; nIndex < lNumTexCoords;nIndex++)
	{
		curObjData.verData[nIndex].uv.x = VerData[nDataIndex++];
		curObjData.verData[nIndex].uv.y = 1.0 - VerData[nDataIndex++];
	}
	//end my code
	return 1;
}
...
{
	short lNumVertices =0;
	aPreviousChunk->mBytesRead += fread(&lNumVertices, 1, 2, mFile);
	aPreviousChunk->mBytesRead += fread(mBuffer, 1, aPreviousChunk->mLength - aPreviousChunk->mBytesRead, mFile);
	// mBuffer now contains a list of vertex coordinates (3 floats)
	
	//add my code
	curObjData.VerCount = lNumVertices;
	float* VerData = (float*)mBuffer;
	if(curObjData.verData == NULL)
		curObjData.verData = new XYZNUV[lNumVertices];

	int nDataIndex  =0;
	for(short nIndex = 0; nIndex < lNumVertices;nIndex++)
	{
		curObjData.verData[nIndex].position.x = VerData[nDataIndex++];
...
std::vector<ObjectData>& Load3ds::GetObject()
{
	return m_Objects;
}
