...
bool FontClass::LoadTexture( WCHAR* filename)
{
	...
	result = m_Texture->initWithFile(filename);
	if(!result)
	{
		return false;
	}
...
void FontClass::BuildVertexArray(void* vertices, const char* sentence, float drawX, float drawY)
{
	...
	for(i=0; i<numLetters; i++)
	{
		...
		else
		{
			...
			vertexPtr[index].uv = D3DXVECTOR2(m_Font[letter].left, 0.0f);
			index++;

			vertexPtr[index].position = D3DXVECTOR3((drawX + m_Font[letter].size), (drawY - 16), 0.0f);  // Bottom right.
			vertexPtr[index].uv = D3DXVECTOR2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = D3DXVECTOR3(drawX, (drawY - 16), 0.0f);  // Bottom left.
			vertexPtr[index].uv = D3DXVECTOR2(m_Font[letter].left, 1.0f);
			index++;
			...
			vertexPtr[index].uv = D3DXVECTOR2(m_Font[letter].right, 0.0f);
			index++;
